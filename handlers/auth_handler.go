...
func AuthHandler(username, password, device_id, ssn string, isInfoSave bool) (string, string, models.User, error) {
	var err error
	...
	err = config.DB.Model(models.User{}).Where("id = ?", res.ID).Update("device_id", device_id).Update("save_login", isInfoSave).Update("last_login", time.Now()).Update("ssn", ssn).Error
